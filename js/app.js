...
function renderProductCard(item, catId = '') {
  ...
  const wishlistBtnHtml = !isRegalo ? `
    <button class="wishlist-btn" data-product-id="${item.id}" onclick="event.stopPropagation(); window.toggleWishlistItem('${item.id}')" aria-label="Guardar en wishlist">
  ...
  return `
    <div class="thumb sp-ticket-wrapper" style="background:var(--ink-2); display:flex; align-items:center; justify-content:center; overflow:visible; position:relative;">
      <style>
        ...
        .sp-ticket-back .sp-ticket-logo { position: absolute; right: 8%; top: 13%; width: 16%; height: auto; }
    ...
    <div class="thumb gc-widget-container" style="background:var(--ink-2); display:flex; align-items:center; justify-content:center; overflow:visible; position:relative; aspect-ratio: 9/10;">
      <style>
        ...
        .gc-logo-img {
            width: 48%;
            height: auto;
            aspect-ratio: 195 / 173; /* crops the transparent padding baked into the source asset */
            object-fit: cover;
